...
typedef	struct		s_scop
{
	...
	int				obj_size;
	int				skybox_size;
	unsigned int	object_texture_id;
	unsigned int	skybox_texture_id;
	t_binded		binded_obj;
	t_binded		cubemap_obj;
}					t_scop;
...
**					cut_string.c
*/
void				cut_face_string(t_tmp_vertex *v, char *line,
									int *faces_count);
...
**					load_obj.c
*/
t_binded			load_obj(t_timer *timer, char *filename,
								int size, short skybox);
void				fill_vertex_array(t_vertex *vertices,
								t_tmp_vertex v, int size);

/*
**					draw.c
*/
void				draw_object(t_scop *s, unsigned int size,
								unsigned int program);
void				draw_skybox(t_scop *s, unsigned int size,
								unsigned int program);
...
**					create_shader.c
*/
int					read_shaders(const char *vertex_path,
									const char *fragment_path);

/*
**					shader_locations.c
*/
void				set_mat4(unsigned int program,
								const char *name, t_mat4 matrix);
void				set_vec3(unsigned int program,
								const char *name, t_vec3 vector);
void				set_int1(unsigned int program,
